// Page component for WithdrawForm.
...
const WithdrawForm = () => {
  ...
  return (
    <div className="pt-4 space-y-5">
      <div className="flex justify-between items-center rounded-xl border border-white/15 bg-slate-900/70 text-xl font-bold px-5 py-4">
        <p>Available balance</p>
        <p>${wallet.userWallet?.balance}</p>
      </div>
      <div className="flex flex-col items-center">
        <h1 className="text-sm text-slate-300">Enter withdrawal amount</h1>

        <div className="flex items-center justify-center ">
          <Input
            onChange={handleChange}
            value={amount}
            className="withdrawInput h-12 border-none outline-none focus:outline-none px-0 text-2xl text-center bg-transparent"
            placeholder="$9999"
            type="number"
          />
        </div>
      ...
      <div>
        <p className="pb-2 text-sm text-slate-300">Transfer to</p>
        <div className="flex items-center gap-5 border border-white/15 bg-white/5 px-5 py-2 rounded-xl">
          <img
            className="h-8 w-8"
